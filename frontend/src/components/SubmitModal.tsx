...
export default function SubmitModal({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl max-h-[90vh] flex flex-col">
        {/* Header */}
        ...
        {step === "success" ? (
          <div className="flex flex-col items-center justify-center gap-4 p-10 text-center">
            ...
            <button
              onClick={onClose}
              className="mt-2 bg-brand-600 text-white px-6 py-2 rounded-xl text-sm font-medium hover:bg-brand-700 transition-colors"
            >
              Close
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="overflow-y-auto px-5 py-4 flex flex-col gap-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white font-semibold py-3 rounded-xl transition-colors mt-1 mb-2"
            >
...
const inputCls =
  "w-full border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-100 bg-white transition-all";
